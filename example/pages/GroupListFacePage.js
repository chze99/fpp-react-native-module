...
import { getGroupInfo,getFace,deleteFace, unbindFace } from "facepass-react-native-module";
        ...
        async function groupInfo() {
            try {
                ...
                const facedata = await Promise.all(_.map(data, async (datas) => {
                    ...
                    return { token: datas, name: dataList.faceName, image: dataList.fileName }
    ...
    return (
        <View>
            {_.map(facegrouplist, (data) => {
                return (<View key={data.token} style={{ display: "flex", flexDirection: "row", paddingTop: 5 ,marginHorizontal: 20, marginVertical: 10  }}>
                    {data.image != "" ?
                        <Image
                    ...
                    <TouchableOpacity style={styles.button} onPress={async () => {
                        try {
                            await unbindFace(data.token, "testi");
                            await AsyncStorage.removeItem(data.token);
                            groupInfo(name)
                        } catch (e) {
                            console.log(e)
                        }
                    }}><Text>Unbind</Text></TouchableOpacity>
                </View>)
            })}
        </View>
    )
